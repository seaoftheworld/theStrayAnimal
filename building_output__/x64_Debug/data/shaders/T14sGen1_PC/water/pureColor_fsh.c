...
void main() {
    ...
    vec4 dummy_color = vec4(0.0, 0.4, 0.8, 1.0);
        ...
        vec4 color_from_normal = texture2D(normalTexture, dudv_factor);  // blocky, wavy
    {
        ...
        // vec3 normal = vec3(color_from_normal.r * 2.0 - 1.0, color_from_normal.g * 2.0 - 1.0, color_from_normal.b * 2.0 - 1.0);
        ...
        // vec3 vertexReflectedLight01 = reflect(-unitVertexToLight01, unitVertexSurface);
        ...
        // 02
        vec3 unitVertexToLight02 = normalize(vertexToLight02);
        vec3 vertexReflectedLight02 = reflect(-unitVertexToLight02, normal);

        // vec3 unitVertexToCamera = normalize(vertexToCamera);
            float specularFactor02 = dot(vertexReflectedLight02, unitVertexToCamera);
            specularFactor02 = max(specularFactor02, 0.0);
            specularFactor02 = pow(specularFactor02, shineDamper);

            vec3 specular02 = lightColor * specularFactor02 * reflectivity;

        // 03
        vec3 unitVertexToLight03 = normalize(vertexToLight03);
        vec3 vertexReflectedLight03 = reflect(-unitVertexToLight03, normal);

            float specularFactor03 = dot(vertexReflectedLight03, unitVertexToCamera);
            specularFactor03 = max(specularFactor03, 0.0);
            specularFactor03 = pow(specularFactor03, shineDamper);

            vec3 specular03 = lightColor * specularFactor03 * reflectivity;


        total_specular = specular00 + specular01 + specular02 + specular03;
    }
    ...
    // gl_FragColor = color_from_normal + vec4(total_specular, 1.0);
    // gl_FragColor = color_from_normal;
    gl_FragColor = dummy_color + vec4(total_specular, 0.0);
}
