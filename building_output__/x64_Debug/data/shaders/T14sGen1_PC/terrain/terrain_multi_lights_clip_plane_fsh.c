...
void main()
{
    ...
    vec4 texture00_color = texture(texture00, tiled_uv) * texture00_proportion;
    vec4 texture01_color = texture(texture01, tiled_uv) * blendMapColor.r;
    vec4 texture02_color = texture(texture02, tiled_uv) * blendMapColor.g;
    vec4 texture03_color = texture(texture03, tiled_uv) * blendMapColor.b;
    ...
    for (int i = 0; i < 8; i++) {
        ...
        vec3 vertexReflectedLight = reflect(-unit_VertexToLight, unit_VertexSurface);
            ...
            // sum_specular = sum_specular + dampedFactor * reflectivity * lightColor[i] / attFactor;
            sum_specular = sum_specular + dampedFactor * lightColor[i] / attFactor;
    }
    sum_diffuse = max(sum_diffuse, 0.2);

    vec4 color = vec4(sum_diffuse, 1.0) * blend_color + vec4(sum_specular, 1.0);
    // gl_FragColor = color;
    out_color = color;
}
