...
const unsigned int NoLightingShader::attr_idx[max_attrNum] = {
    ATTR_IDX0_POS,
    ATTR_IDX1_UV, 
    ATTR_IDX2_NORMAL
};

const unsigned int NoLightingShader::attr_stride[max_attrNum] = {
    ATTR_IDX0_POS_STRD,
    ATTR_IDX1_UV_STRD, 
    ATTR_IDX2_NORMAL_STRD
};

const unsigned int NoLightingShader::attr_offset[max_attrNum] = {
    0, 
    attr_stride[0],
    attr_stride[0] + attr_stride[1]
};

const unsigned int NoLightingShader::all_in_one_stride = 
...
void NoLightingShader::getAllUniformLocations() {
    ...
    printf("    unif_loc: transMat-%d, viewMat-%d, projMat-%d, sampler-%d\n\n", 
        getTransformMatLoc(), getViewMatLoc(), getProjMatLoc(), texture_sampler_loc);
}
